...
/**
 * Generate VueJS Resource for the given loopback application.
 *
 * ```js
 * var generateServices = require('loopback-sdk-vuejs').services;
 ...
 * @param {string=} vueResourceName 
 *        A name for the generated vue-resource module.
 *   Default: `lbServices`.
 ...
 */
module.exports = function generateServices(app, vueResourceName, apiUrl) {
  vueResourceName = vueResourceName || 'lbServices';
  apiUrl = apiUrl || '/';
...
function describeModels(app) {
  ...
  app.handler('rest').adapter.getClasses().forEach(function(c) {
    ...
    // Because all $resource methods are static (non-prototype) in vue-resource,
...
function buildScopeMethod(models, modelName, method) {
  ...
  // build a reverse record to be used in vue-resource
